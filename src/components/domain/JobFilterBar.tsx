...
const KEISHIKI_OPTIONS = ["すべて", "請負", "応援（常用）"];
const JISU_OPTIONS = ["すべて", "1次下請", "2次下請", "3次下請"];
...
export function JobFilterBar({
  ...
  const hasFilter =
    !!initial.q || initial.area !== "すべて" || initial.industry !== "すべて" || initial.keishiki !== "すべて" || initial.jisu !== "すべて";
  ...
  const push = (next: Partial<Filters>) => {
    ...
    if (merged.keishiki !== "すべて") params.set("keishiki", merged.keishiki);
    if (merged.jisu !== "すべて") params.set("jisu", merged.jisu);
      ...
      {open && (
        <div className="p-3 mt-2 rounded-sm" style={{ background: C.kami, border: `1px solid ${C.keisen}` }}>
          ...
          <div className="mb-2">
            ...
            <Pills options={JISU_OPTIONS} value={initial.jisu} onChange={(v) => push({ jisu: v })} />
          </div>
          <div className="mb-2">
            <span className="block text-[11px] font-bold mb-1" style={{ color: C.usu }}>
              募集形態
            </span>
            <Pills options={KEISHIKI_OPTIONS} value={initial.keishiki} onChange={(v) => push({ keishiki: v })} />
          </div>
          <div className="mb-2">
            <span className="block text-[11px] font-bold mb-1" style={{ color: C.usu }}>
              エリア
