...
const DONE = [
  ...
  ["Step7", "Supabaseクライアント・ログイン/会社登録（src/lib/supabase, /login, /signup）"],
];
  ...
  return (
    <div className="min-h-screen" style={{ background: C.yojo }}>
      <div className="sticky top-0 z-20 flex items-center px-3 h-14" style={{ background: C.sumi }}>
        <h1 className="text-[17px] font-extrabold" style={{ color: "#fff" }}>ゲンバノワ 実装状況</h1>
      </div>
      <main className="max-w-md mx-auto p-3">
        ...
        <p className="text-[11px] mt-2" style={{ color: C.usu }}>
          Supabaseプロジェクトを .env.local に設定すると /login から使えます。
        </p>
      </main>
    </div>
  );
